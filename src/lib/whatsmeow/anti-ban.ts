 ...
 * La cola deja de martillar la línea cuando Meta ya la marcó.
 */
...
export function isWhatsappBanLikeError(error: unknown) {
  const msg = String(error || "").toLowerCase();
  if (!msg.trim()) return false;
  return /banned|blocked|forbidden|\bspam\b|rate.?limit|too many|not.?authorized|\b403\b|\b401\b|iq.?error|temporarily.?banned|connection.?closed/.test(
    msg
  );
}

export function isWhatsappPermanentSendError(error: unknown) {
  const msg = String(error || "").toLowerCase();
...
export function isWhatsappTransientDisconnect(error: unknown) {
  const msg = String(error || "").toLowerCase();
  if (!msg.trim()) return false;
